...
This is the implementation of the `ADDI` operation from the SpecOps M68K module. It checks whether the instruction's given operands are an integer and a location (in the register file or in memory), returning an error if not. It uses a macro called `(determine)` which checks whether the width specification and operands given for the instruction are valid, and if so derives the numbers encoding those values for the operands. The encoded width, addressing mode and index are used as arguments to a macro called `(masque)` that generates code composing the instruction word.

If one of the instruction parameters is invalid, the code generated by `(determine)` provides for an error message describing the correct format. For example:

```
* (assemble *assembler-prototype-m68k* (:addi :b 10 50))

Invalid operand(s) for instruction ADDI. Format: 
ADDI W   - width : any
     OP0 - immediate value : 32 bits
     OP1 - general purpose register : any
           memory access : (An) (An)+ -(An) (d16,An) (d8,An,Xn) ABS.W ABS.L
   [Condition of type SIMPLE-ERROR]
```

The code above attempts to call the `ADDI` instruction with two immediate values as operands. This won't work so an error message appears describing valid input for `ADDI`.
...
* (asdf:load-system 'specops.system-z) (in-package :specops.system-z)
...system loads...
...
Let's take a closer look at the macros above - these aren't the `(masque)` seen before, but something called `(mqbase)`. This is essentially a currying macro that generates another macro producing functions built around `(masque)`. The `(specop-z)` macro takes a formatting function like `zformat-rr` generated by `(mqbase)` and uses it as the basis to construct an assembling function for an instruction like `AR`. The bitfields in the string given as the fourth argument to the macro are populated on the basis of the forms expressed in the fifth and sixth arguments.


The `:static` forms in the fifth argument express static fields within the bitfield that will be the same regardless of the specific operands given to an instruction of a given format, those those fields are fixed for a given instruction and they don't need to be evaluated during the assembly process. The `:determine-by` fields above populate `(determine)` macro forms in the output code. As for M68K, this macro both validates and encodes operands passed as input to instructions, as well as formatting error messages explaining the accepted types of input for invalid instructions.
...
RISC (Reduced Instruction Set Computer) hardware architectures are known for executing code at consistently high speeds, but this often comes at the expense of elegance in instruction design. Platforms like ARM and M68K can be haphazard in their organization of bit fields so as to fit all the needed information into a fixed width. A standout in this sector is the SuperH architecture, originally designed by Hitachi. This line of CPUs powered the Sega Saturn, Sega Dreamcast and a wide range of arcade game hardware along with mechanical control systems in many vehicles. SuperH uses mostly 16-bit instructions (with a few 32-bit extensions added in later revisions) with all fields nibble-aligned.
...
Another quality of the Z80 architecture is that it has two alternate instruction tables that mirror the functionality of the base table, but applied to different registers. These alternate tables have opcodes that start with the prefixes `#xDD` and `#xFD`. Any instruction in the base table that addresses the `HL` register has an alternate form in the `#xDD` table that addresses the `IX` register and another in the `#xFD` table that addresses the `IY` register.
